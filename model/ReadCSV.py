...
class CSV_Reader:
    ...
    def get_dict(self, csv_file_name):
        ...
        if os.path.isfile(csv_file_name): 
            ...
            self.term_list += [str(t) for t in csv_file.readline().split(',')[1:]]
            for line in csv_file:
                doc_id, word_freq_list = line.split(',')[:1], [int(i) for i in line.split(',')[1:len(line.split(','))-2]]
        ...
        if len(self.term_list) > 0:
            ...
            for doc in self.doc_list:
                for term in self.term_list:
                    outfile.write(','+weight_dict[term])
                outfile.write('\n')
        outfile.close()
